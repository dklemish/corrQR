  ...
  for(k = 0; k < q; k++){
    ...
    //** 4. ||v_l||^2 for each quantile l (1:L)
     ...
     if(vNormSq.min() > 0.0){
       ...
       // a.print("a");
       // Rcout << "a.row = " << a.n_rows << "; a.col = " << a.n_cols << std::endl;
       ...
       for(i = 0; i < n; i++){
         ...
         if(Q_L == -std::numeric_limits<double>::infinity() ||
           ...
           llmat.at(i,k) += -1*log((1-alpha)*b0dot[l-1]*(1+aTilde.at(i,l-1)) +
